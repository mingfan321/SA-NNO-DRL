...
class OP(object):
    ...
    def get_costs(dataset, pi):
        ...
        prize_with_depot = torch.cat(
            ...
            1
        )[:, None, :].expand(pi.size(0), pi.size(1), -1)
        p = prize_with_depot.gather(-1, pi)

        # Gather dataset in order of tour
        loc_with_depot = torch.cat((dataset['depot'][:, None, :], dataset['loc']), 1)[:, None, :, :].expand(pi.size(0), pi.size(1), -1, 2)
        ...
        length = (
            ...
            + (d[:, :, 0] - dataset['depot'][:, None]).norm(p=2, dim=-1)  # Depot to first
            + (d[:, :, -1] - dataset['depot'][:, None]).norm(p=2, dim=-1)  # Last to depot, will be 0 if depot is last
        )
        assert (length <= dataset['max_length'][:, None] + 1e-5).all(), \
            "Max length exceeded by {}".format((length - dataset['max_length'][:, None]).max())
...
def generate_instance(size, prize_type, num_samples):
    # Details see paper
    ...
    for i in range(num_samples):
        location = torch.FloatTensor(uav_num * size, 2).uniform_(0, 1)
        depot = torch.Tensor([0.5, 0.5])
        lab = location - depot
        angle = torch.atan2(lab[:, 1], lab[:, 0])
        angle_sort = angle.argsort()
        relocation = location[angle_sort]
        for j in range(uav_num):
            loc = relocation[j * size: (j + 1) * size] if j < uav_num - 1 else relocation[j * size:]
            # prize_type = 'unif'
            # Methods taken from Fischetti et al. 1998
            if prize_type == 'const':
                prize = torch.ones(size)
            elif prize_type == 'unif':
                prize = (1 + torch.randint(0, 100, size=(size,))).float() / 100
...
class OPDataset(Dataset):
    ...
    def __init__(self, filename=None, size=50, num_samples=1000000, offset=0, distribution='const'):
        ...
        if filename is not None:
            ...
            self.data = generate_instance(size, prize_type, num_samples)
            # self.data = [
