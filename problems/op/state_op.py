...
class StateOP(NamedTuple):
    ...
    @staticmethod
    def initialize(input, pomo_size, visited_dtype=torch.uint8):
        depot = input['depot']
        loc = input['loc']
        prize = input['prize']
        ...
        return StateOP(
            ...
            prev_a=torch.zeros(batch_size, pomo_size, dtype=torch.long, device=loc.device),
            visited_=(  # Visited as mask is easier to understand, as long more memory efficient
                # Keep visited_ with depot so we can scatter efficiently (if there is an action for depot)
                torch.zeros(
                    batch_size, pomo_size, n_loc + 1,
                    dtype=torch.uint8, device=loc.device
                )
                if visited_dtype == torch.uint8
                else torch.zeros(batch_size, pomo_size, (n_loc + 1 + 63) // 64, dtype=torch.int64, device=loc.device)  # Ceil
            ),
            lengths=torch.zeros(batch_size, pomo_size, device=loc.device),
            cur_coord=input['depot'][:, None, :].expand(batch_size, pomo_size, loc.size(-1)),  # Add step dimension
            cur_total_prize=torch.zeros(batch_size, pomo_size, device=loc.device),
    ...
    def get_k_nearest_neighbor(self):
        k_nn = (self.coords[:, 0, :][:, None, :] - self.coords[:, 1:, :]).norm(p=2, dim=-1).sort(dim=-1)[1]
        mask_idx = (self.coords[:, 0, :][:, None, :] - self.coords[:, 1:, :]).norm(p=2, dim=-1) > self.max_length[:, 1:]
        mask_knn = torch.gather(mask_idx, 1, k_nn)
        k_nn += 1
        k_nn[mask_knn] = 0
    ...
    def update(self, selected):
        ...
        # Update the state
        # selected = selected[:, :]  # Add dimension for step
        prev_a = selected

        # Add the length
        cur_coord = self.coords[self.ids, selected]
        lengths = self.lengths + (cur_coord - self.cur_coord).norm(p=2, dim=-1)  # (batch_dim, pomo_size)
    ...
    def get_mask(self):
        ...
        exceeds_length = (
                self.lengths[:, :, None] + (self.coords[self.ids, :, :] - self.cur_coord[:, :, None, :]).norm(p=2,
                                                                                                              dim=-1)
